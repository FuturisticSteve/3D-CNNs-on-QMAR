...
root_path = Path('I:\Optical-Flow-tvl1')
...
training_save_filename = disease_kind + '-' + motion_kind + '-training-flow-fold' + str(fold_number) + '.npy'
training_save_filepath = root_path / dataset_name / training_save_filename
testing_save_filename = disease_kind + '-' + motion_kind + '-testing-flow-fold' + str(fold_number) + '.npy'
...
image_list_temp = [[] for i in range(6)]
paths = sorted(normal_path.rglob("*.npy"))
...
for i in range(len(image_list_normal)):
    training_list[0].extend(image_list_normal[i][:int(target_frame_num/clip_duration/6)])
