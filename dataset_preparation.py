...
while target_frame_num%clip_duration != 0 or target_frame_num%6 != 0:
    target_frame_num += 1
            ...
            if n_compansation>=0:
                for j in range(math.ceil(n_compansation / view_count)):
                    if 1+j+clip_duration<len(items):
                        ...
                        slided_clip = image_list_temp[i][int((1+j+clip_duration)%len(items)):int((1+j+clip_duration)%len(items)+clip_duration)]
...
normal_test_sample = normal_test_sample['folder name'].drop_duplicates().values[0]
normal_list = [x for x in normal_list if normal_test_sample not in x]
...
for i, items in enumerate(image_list_temp):
    ...
    while count<target_frame_num/clip_duration/6:
        clip = temp[k]
        k += 1
        if isClip(clip):
            count += 1
            training_list[0].append(clip)
